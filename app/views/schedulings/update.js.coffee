<%= decorate resource.concurrent do |filter| %>
  ...
  <% else %>
    $("#<%= filter.scheduling_form_id %> form#edit_scheduling_<%= resource.id %> .errors").remove()
    $("#<%= filter.scheduling_form_id %> form#edit_scheduling_<%= resource.id %>").append('<p class="alert alert-error errors"><%=j resource.errors.full_messages.to_sentence %></p>')
  <% end %>
<% end %>
